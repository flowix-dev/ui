...
export default function AssistantChatPage() {
  ...
  const assistantChats = useMemo(
    () => chats.filter((chat) => chat.assistantId === assistantId),
    [chats, assistantId],
  );
  ...
  useEffect(() => {
    ...
    if (
      currentChatId &&
      assistantChats.some((chat) => chat._id === currentChatId)
    ) {
      return;
    }
    const first = assistantChats[0];
    dispatch(selectChat(first._id));
    dispatch(fetchChat(first._id));
  }, [assistantChats, currentChatId, dispatch]);
  ...
  const handleCreateChat = useCallback(async () => {
    const result = await dispatch(createChat({ assistantId }));
